...
> **Status: M1 + M2 + M3 in the tree, M4 started; M5 plan.** M1's wrist force/torque sensor,
> M2's force-regulated insertion and M3's Cartesian admittance (compliant) control are
> implemented and tested, and M4's actuated gripper has a first shipped slice (closed-loop
> grasp-force control + a friction grasp with no weld); the rest of M4 and M5 are plan (see
> the status of each below). This scopes how
...
- **Verify — commanded-stiffness curve + push-and-yield.** Driven by a supplied wrench (the control law in isolation), the TCP yield settles to `e = F/K` across a 16× stiffness sweep — K = 200/400/800/1600/3200 N/m → 40/20/10/5/2.5 mm at F = 8 N, `e·K = 8.0 N` at every point — and a per-axis K = [400, 1600, 400] gives e = [20, 5, 0] mm (compliant x/z yield, stiff y holds), returning to ~0 when the wrench is removed. A **push-and-yield** with a *real* force on the wrist read back through the M1 sensor (+8 N) yields the TCP ~21 mm along the pushed axis and returns to the nominal pose on release (sensor-in-the-loop). Reproduce: `python sim/eval_admittance.py --model .../skt_v3`.
- **Risk (addressed):** admittance stability vs the inner position-loop bandwidth — symplectic integration + critical damping + the gravity feed-forward keep it stable across the whole sweep. **Also shipped — bimanual compliant carry** (`sim/carry.py` `CompliantCarry`, `sim/test_carry.py`): the object-level generalisation of demo_dual_carry's 1-D tanh load-equaliser — both arms hold a shared bar and one Cartesian admittance at the object frame (driven by the *summed* wrist wrenches, whose internal squeeze cancels; an equal offset to both wrists preserves the closed kinematic loop) yields the whole assembly to a disturbance and returns it. A 10 N push yields the object ~20 mm along the pushed axis and it springs back, the bar never dropped. **Also in the cockpit:** `RobotBridge` gains a selectable contact response — `set_contact_mode("stop"|"compliant")` — where *compliant* yields the TCP via the same admittance law (inlined, driven by the cockpit's existing `_tcp_force` joint-torque estimate) instead of latching the soft-stop; the default stays `stop`, so the existing reflex and its e2e guard are byte-for-byte unchanged (`tools/skate_commander/test/test_compliant.py`; all 84 cockpit tests green). **Remaining (small):** surfacing the toggle as a UI button / WS command. **Effort:** medium; the main new control theory.

### M4 — Real grasp model  *(actuated gripper)* — **FIRST SLICE SHIPPED**
- **Status — first slice shipped (opt-in, self-contained).** `sim/make_gripper_scene.py` builds `skt_v3_gripper.xml` — an actuated parallel-jaw gripper (two pads on coupled prismatic joints, a force/motor actuator, real friction, a pad touch sensor) grasping a D20 peg — and `sim/gripper.py` (`Gripper`) does **closed-loop grasp-force control**: `close_to_force` regulates the *measured* pad force (not the raw motor command) to a target. Verified by `sim/test_gripper.py` (in CI) and `sim/eval_gripper.py`: the measured grasp tracks the target across 2–5 N (2 N→2.00, 4 N→4.00, monotone), and once the temporary world-pin is released the peg is held by **friction alone — no weld** (dz ≈ −2 mm, grasp force persists). The scene is self-contained (no robot meshes), so the arm's control/collision/cell models and all their tests stay untouched. **Still to do for M4:** mount the jaws on the wrist's `a7` and carry a part through the full cycle weld-free; a grasp-slip curve (payload-until-slip vs grasp force) — a rigid cylinder ejects above ~5 N (MuJoCo's stiff friction-grasp frontier), which is exactly what that tuning targets.
