...
- **Also shipped:** a `benchmark.py` `insert_m2` task on the new controller (bimanual, scored on the base-recoil-invariant oracle), and the peg-tilt (θ) tolerance above. **Remaining M2 (small):** the spec's round chamfered H9 bore (the current square blind pocket is the v1 stand-in). **Risk (addressed):** contact stiffness (`solref`/`solimp`) tuning — the rigid `fixture_base` weld keeps the sweep deterministic. **Effort:** medium; core contact-control work.
