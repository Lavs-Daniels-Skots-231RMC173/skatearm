...
- `benchmark.py` — **bimanual benchmark suite**: headless, seeded, quantitative metrics over N trials for five tasks — reach · carry · hand-off · peg-insert · force-regulated peg-insert (`insert_m2`) — reusing the same primitives; `--json` writes a full report (sample: `benchmark_results.json`). Each task draws its disturbances from its **own** stream (`benchmark.task_rng`), so `--tasks insert` measures the same thing the full-suite run measured — the property the suite exists for, and one a single shared generator quietly does not have. `handoff` is the one task on the jaws scene, and the report records the scene per task rather than leaving it to be inferred. `test_benchmark.py` is the smoke test.
- `handoff.py` — the **hand-off controller** `benchmark.task_handoff` drives: one hand takes the peg out of the other in mid air, on M4's jaws. Every offset is derived from the compiled scene rather than typed — the tool length off `make_gripper_cell`, the peg's and the pad plates' half-lengths off the model, the receiver's upside-down tool frame off `sequencer.R_DOWN` — so it cannot drift from the geometry it describes. Where the two hands meet is measured, not chosen: a peg lying along world **x** is a right-arm workspace boundary and the grasp row `y = 0.44` is out of reach at every height tried, so the part stands along world **z**.
...
| `handoff` | one hand takes the peg **out of the other**, in mid air, on M4's jaws — the giver's grasp aim is offset like `carry`'s, so it picks the peg up crooked and the receiver has to cope | **5/5**, peg settles 0.4 mm when the giver opens, peg tilt ≤0.02°, receiver holding 11.9 N, pad plates never touch (gap 3.9–4.1 mm) |
| `insert` | full bimanual peg-in-hole (offset grasps, carry, align, force-guarded descent) | **5/5**, depth 18.5 mm (target 18), peg tilt 1.2–1.4°, no τ-abort |
| `insert_m2` | the same staging, but the M2 **force-regulated** controller with spiral search absorbs an injected residual xy misalignment (≤2.5 mm) | **5/5**, peg-in-base 23.7 mm, peg tilt 0.2–0.8°, peak wrench 4.8–5.9 N (abort 9), no abort |

Committed report: [`benchmark_results.json`](benchmark_results.json) — regenerate
with the command above (defaults to all five tasks, seed 0). It carries a `meta`
node saying which seed, how many trials and which MuJoCo produced it, and a
`scene` per task, because the tasks no longer all run the same model.

The **hand-off** is no longer deferred, and it is the one task that cannot run on
the default cell: there a grasp is a magnetic weld, and two welds on one body
fight rather than pass it, so `handoff` runs the `--gripper` scene and the other
four keep the weld cell. That makes the scene part of the result — `add_gripper`
sets an elliptic friction cone on the whole model, so a newton from one scene is
not a newton from the other — which is why the report names it per task instead
of leaving a reader to infer it from the task's name. What the run shows is that
the pass itself is not the hard part: the receiver closes on the peg while the
giver still holds it, the two pad plates stop 3.9–4.1 mm apart by construction,
and the peg settles 0.4 mm when the giver opens. `test_benchmark.py` is the smoke
test (one trial of each task, plus the proof that a task's numbers do not depend
on which other tasks ran beside it).
