...
training seed) plus the ``baseline_metrics.json`` from ``baseline_reach.py``,
computes the mean +/- std across seeds, prints the results table, writes
``summary.json``, and redraws the accuracy chart (light + dark variants) — i.e.
the exact numbers and figure used in the top-level README.

    python aggregate_reach.py seed0.json seed1.json seed2.json baseline.json

Any argument whose filename contains "baseline" is the no-vision baseline; the
rest are per-seed ACT rollouts. Charts go to ``OUT_DIR`` (default: the repo's
...
args = sys.argv[1:]
if not args:
    sys.exit("usage: python aggregate_reach.py <seed*.json> ... <baseline.json>")
seed_files = [a for a in args if "baseline" not in os.path.basename(a).lower()]
base_files = [a for a in args if "baseline" in os.path.basename(a).lower()]
...
if base:
    print("BASE  R %.1f cm   L %.1f cm   success@8cm %.0f%%" % (summary["base_R"], summary["base_L"], summary["base_succ"]))


def draw(dark):
    if dark:
        BG, FG, GRID, blue, gray = "#0d1117", "#c9d1d9", "#8b949e", "#58a6ff", "#8b949e"
    else:
        BG, FG, GRID, blue, gray = "white", "#1f2328", "#c9ccd1", "#2563EB", "#9BA2AC"
    orange = "#F59E0B"
    ...
    ax.axhspan(0, 3, color="#16a34a", alpha=0.20 if dark else 0.12, zorder=0,
               label="within marker radius (3 cm)")
    ax.axhline(8, color=FG, ls=":", lw=1.1, alpha=0.55)
    ax.text(1.46, 8.25, "8 cm success threshold", fontsize=8, color=FG, alpha=0.7, ha="right")
    x = np.array([0.0, 1.0])
    w = 0.34
    actm, acts = [sR.mean(), sL.mean()], [std(sR), std(sL)]
    ax.bar(x - w / 2, actm, w, yerr=acts, capsize=5, color=[orange, blue],
           error_kw=dict(ecolor=FG, lw=1.3), label="ACT policy (%d seeds)" % len(seeds), zorder=3)
    if base:
        ax.bar(x + w / 2, [summary["base_R"], summary["base_L"]], w, color=gray, alpha=0.55,
               hatch="//", edgecolor=GRID, label="no-vision baseline", zorder=3)
    ax.scatter(np.full(len(seeds), x[0] - w / 2), sR, s=26, color=BG, edgecolor=FG, lw=1.1, zorder=5)
    ax.scatter(np.full(len(seeds), x[1] - w / 2), sL, s=26, color=BG, edgecolor=FG, lw=1.1, zorder=5)
    for xi, mv in zip(x - w / 2, actm):
        ax.text(xi, mv + max(acts) + 0.5, "%.1f" % mv, ha="center", fontsize=9, fontweight="bold", color=FG)
    if base:
        for xi, mv in zip(x + w / 2, [summary["base_R"], summary["base_L"]]):
    ...
    top = max([summary.get("base_R", 0), summary.get("base_L", 0)] + actm)
    ax.set_ylim(0, top * 1.18)
    ax.set_title("ACT vs no-vision baseline - %d training seeds x %d unseen rollouts (mean +/- std)"
                 % (len(seeds), n_eval), fontsize=10.5, fontweight="bold", color=FG)
