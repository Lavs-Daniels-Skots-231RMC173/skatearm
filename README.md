...
<sub>→ Phase 1's τ-watchdog insert is since replaced by real contact-force control — the cycle above runs the force-regulated S4. The numbers above are the **weld** cycle, which stays the default. The actuated gripper is now wired into the live cycle too, on the **right** hand: `make_cell_scene.py --gripper` builds an opt-in scene where S1's receptivity is a measured 12 N grasp force rather than a weld toggle, the peg is carried on friction (0.3 mm slip), and S4 releases by *opening the jaws* — camera-ACCEPT, 38.96 s, `sim/test_cell_gripper.py` in CI. The **left** hand still welds the base, so the cell is half-converted, not weld-free. See the **[Manipulation core](#manipulation-core-contact-force-control)** below.</sub>
...
| **M4** · actuated gripper | parallel-jaw gripper, grasp-force control, a grasp-slip curve, a weld-free grasp-carry-**place** on the arm — and those jaws driving the **right hand of the live S0–S7 cycle** (opt-in scene; the left hand still welds the base) | grasp tracks target **2–5 N**; slip payload grows with grasp force; part carried (~5 mm drift) and **placed in a bin — no weld**. In the cycle: 12 N grasp with the weld inactive, 0.3 mm slip over the carry, insert to 22.1 mm at 1.86 N peak, jaws **open** to release, camera **ACCEPT**, **38.96 s** | [test_gripper.py](sim/test_gripper.py) · [test_gripper_arm.py](sim/test_gripper_arm.py) · [test_cell_gripper.py](sim/test_cell_gripper.py) · [data](sim/eval_data/gripper.json) |
