...
<sub>→ Phase 1's τ-watchdog insert is since replaced by real contact-force control — the cycle above runs the force-regulated S4. The numbers above are the **weld** cycle, which stays the default and is the one the cameras verify. The actuated gripper is now wired into the live cycle too, on **both** hands: `make_cell_scene.py --gripper` builds an opt-in scene where S1's receptivity is a measured grasp force rather than a weld toggle, the left hand picks the base off the table and sets it down instead of holding it in mid-air, the peg is carried on friction (0.42 mm slip), and S4 releases by *opening the jaws* — a cell with no `weld` active in it anywhere, 22.1 mm insert, 75.8 s, `sim/test_cell_gripper.py` in CI. That cell is gated by the sim oracle, not the cameras: converting the left hand put the tool in the overhead camera's line of sight. See the **[Manipulation core](#manipulation-core-contact-force-control)** below.</sub>
...
| **M4** · actuated gripper | parallel-jaw gripper, grasp-force control, a grasp-slip curve, a weld-free grasp-carry-**place** on the arm — and those jaws driving **both hands of the live S0–S7 cycle** (opt-in scene, no `weld` active anywhere in it) | grasp tracks target **2–5 N**; slip payload grows with grasp force; part carried (~5 mm drift) and **placed in a bin — no weld**. In the cycle: six real grip/release operations with both welds inactive throughout, left picks the base off the table (12.00 N) and sets it down, 0.42 mm slip over the carry, insert to **22.1 mm at 2.98 N** peak, jaws **open** to release, left re-grips the unit (0.43 mm drift) and bins it — **22.12 mm / 1.90° / 1.24 mm ACCEPT**, **75.8 s** (oracle-gated; see the QC note below) | [test_gripper.py](sim/test_gripper.py) · [test_gripper_arm.py](sim/test_gripper_arm.py) · [test_cell_gripper.py](sim/test_cell_gripper.py) · [data](sim/eval_data/gripper.json) · [QC-cost eval](sim/eval_qc_occlusion.py) · [data](sim/eval_data/qc_occlusion.json) |

<div align="center">
  <img src="docs/img/cell_cycle_gripper.gif" width="420px" alt="The weld-free work cell: left jaws pick the base off the table and set it down, right jaws grip the peg for the insert">
  <img src="docs/img/qc_occlusion_jaws.png" width="420px" alt="The QC inspection window on both paths: the weld path sees the peg and the pocket rim, the weld-free path sees the left tool">
  <br>
  <em>Left — the <strong>weld-free cycle</strong> (excerpt): the left hand <em>picks the base off the table</em> and sets it at the assembly station, because jaws cannot hold it in mid-air the way the weld did. Right — <strong>what that cost</strong>, measured in the same 300 px inspection window: the weld path sees 1116 peg px and 7581 rim px and ACCEPTs; the weld-free path sees the left tool — 0 peg px, 89 % of the rim gone — and REJECTs, on a unit the pose oracle accepts on <em>both</em> paths (<a href="sim/eval_data/qc_occlusion.json">raw counts</a>, re-derived in CI).
  HD video: <a href="docs/video/cell_cycle_gripper.mp4">cell_cycle_gripper.mp4</a></em>
</div>

> **The honest QC boundary.** The weld-free cell is gated by the **sim oracle**, not by the cameras, and the reason is the conversion itself. The left tool has to approach the base top-down — the pocket faces up and the base's 60 mm length exceeds the 41.61 mm jaw gap, so the orientation is forced — which parks the left wrist between `qc_top` and the unit at exactly the pose QC inspects. Running the unchanged pipeline at its calibration resolution on both paths gives ACCEPT on the weld path and REJECT on the jaw path, so the flip is the cell and not the measurement. **The last weld was the one holding the part in the overhead camera's line of sight.** Restoring a camera gate here needs in-situ inspection optics aimed at the assembly station — a second camera pair, not a re-calibration — and until that ships, "weld-free" and "camera-verified" are true of this project on *different* cells. The committed camera-QC numbers stay on the weld path, where they were earned.
...
| [`skate_moveit_config`](tools/skate_moveit_config/) | MoveIt 2 config for the bimanual chains — SRDF from the URDF, OMPL planning, `FollowJointTrajectory` bridge to the driver | ✅ **built & verified on ROS 2 Jazzy** |
| [`skate_ros2_control`](tools/skate_ros2_control/) | **ros2_control** hardware interface — C++ `SystemInterface` + per-arm `JointTrajectoryController`s, no Python bridge | ✅ **verified on ROS 2 Jazzy** |
